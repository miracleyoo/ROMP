...
def pack_data(vertex_save_dir, data_folder, split, annots_path):
    ...
    for af_ind, annot_file in enumerate(all_annot_paths):
        ...
        np.savez(annots_path.replace('.npz','_{}.npz'.format(af_ind)),annots=annots)
    np.savez(annots_path,annots=annots)   
    np.savez(os.path.join(data_folder,'subject_IDs_dict_{}.npz'.format(split)), subject_ids=smpl_subject_dict) 
    return annots
