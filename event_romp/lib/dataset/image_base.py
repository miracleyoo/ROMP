...
if args().learn_AE:
    from maps_utils.target_generators import JointsGenerator, HeatmapGenerator
...
class Image_base(Dataset):
    ...
    def get_item_single_frame(self,index):
        """
        Get single frame data. Things it does:
        1. Load image and annotations
        2. Augment image and annotations
        3. Generate centermap, heatmap, 3D pose, 2D pose, etc.
        ...
        # get_image_info is a dataset-specific function that loads image and annotations
        ...
        # image: Fully augmented image (also padded to square)
        # image_wbg: Original image padded by 0 to square shape (no other augmentations)
        # full_kps: Augmented 2D pose
        # offsets: the H, W of the augmented image, and the trbl of the crop box and padding
        ...
        # image: Fully augmented image (also padded to square) (same as above input image)
        # dst_image: `image` resized to `input size`
        # org_image: non-augmented but padded to square image (image_wbg) resized to `input size`
