...
def image_crop_pad(image, kp2ds=None, crop_trbl=(0,0,0,0), bbox=None, pad_ratio=1., pad_trbl=None, draw_kp_on_image=False):
    ...
    return:
        image_aug: np.array, size H x W x 3, the augmented image.
        kp2ds_aug: np.array, size N x K x 2/3, the augmented 2D joints of N Joints
        offset: np.array, size 10, represent the H, W of the augmented image, and the trbl of the crop box and padding
    '''
    if bbox is not None:
...
def image_pad_white_bg(image, pad_trbl=None, pad_ratio=1.,pad_cval=255):
    """
    Pad the image to reach the target aspect ratio, and fill the padded area with white background.
    Input args:
        image : np.array, size H x W x 3
        pad_trbl : tuple, size 4, represent the pad size on top, right, bottom, left side, Each entry may be a single int.
        pad_ratio : float, ratio = width / height
    Return:
        padded image: np.array, size H x W x 3
        pad_trbl: np.array/list/tuple, size 4, represent the pad size on top, right, bottom, left side, Each entry may be a single int.    
    """
    if pad_trbl is None:
...
def process_image(originImage, full_kp2ds=None, augments=None, is_pose2d=[True], random_crop=False):
    """
    Process the image and the corresponding keypoints.
    Input args:
        originImage : np.array, size H x W x 3
        full_kp2ds : list of np.array, each np.array is size N x 3, N is the number of keypoints, 3 is the x,y,v
        augments : tuple, size 4, represent the rotation, flip, crop_bbox, img_scale
        is_pose2d : list of bool, each bool represent whether the corresponding kp2ds is 2d pose or not
        random_crop : bool, whether to do random crop
    Return case 1:
        orgImage_white_bg: np.array, size H x W x 3, the image padded with zeros to reach the target aspect ratio.
        pad_trbl: np.array, size 4, represent the pad size on top, right, bottom, left side, Each entry may be a single int.
    Return case 2:
        image_aug: np.array, size H x W x 3, the image after augmentation
        orgImage_white_bg: np.array, size H x W x 3, the image padded with zeros to reach the target aspect ratio.
        kp2ds_aug: list of np.array, each np.array is size N x 3, N is the number of keypoints, 3 is the x,y,v. The keypoints after augmentation.
        offsets: np.array, size 10, represent the H, W of the augmented image, and the trbl of the crop box and padding
    """
